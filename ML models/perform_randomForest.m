% This script is used to train a random forest classifier via a ten-fold cross
...
for l = 1:model_trial % iterations per model trial
    ...
    model = TreeBagger(21,num,label); 
    % NOTE: The number of trees (21) considered for training the random
    % forest classifier was determined using a sweep from 1 to 100 trees
    % using the "randomForest_optimizer.m" script. You may try to load and
    % plot "treeBag_perf_sweep.mat" to view the performance plot of the
    % performed sweep.
...
RF_mean_perf = {'AUC','accuracy','PPV','NPV','SR','RR';mean_perf(1),mean_perf(2),mean_perf(3),mean_perf(4),mean_perf(5),mean_perf(6)}
RF_std_perf = {'AUC','accuracy','PPV','NPV','SR','RR';std_perf(1),std_perf(2),std_perf(3),std_perf(4),std_perf(5),std_perf(6)}
